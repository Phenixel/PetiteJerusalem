...
describe("les villes du Nord, où l'avis par degrés s'arrête", () => {
  ...
  it("gardent leurs six jeûnes et tous leurs Chabbats", () => {
    const anomalies: string[] = [];
  ...
  it("expliquent chaque horaire absent, sans jamais le confondre avec la nuit polaire", () => {
    const anomalies: string[] = [];
    for (const place of NORTH) {
      let explained = 0;
      for (let i = 0; i < 365; i++) {
        ...
        if (missing !== (gap !== null)) {
          anomalies.push(`${tag} : ${missing ? "horaire absent sans note" : "note sans absence"}`);
        }
        if (!gap) continue;
        explained++;
        ...
        if (gap.reason !== "degrees") anomalies.push(`${tag} : note « ${gap.reason} » inattendue`);
