...
class WidgetService {
  ...
  private async push(): Promise<void> {
    try {
      ...
      // l'app avant son premier rendu. Celui de la montre en dépend aussi, il
      // part donc avec eux.
      const [
        { buildDailyReadingWidgetPayload, buildLibraryWidgetPayload, buildZmanimWidgetPayload },
        { buildWatchPayload },
      ] = await Promise.all([import("./widgetPayloads"), import("./watchPayloads")]);
