...
<template>
  <AppModal :open="show" :label="title" @close="dismiss">
    ...
    <div v-if="showBenefits" class="py-5">
      <div class="space-y-3">
        <div class="flex items-center gap-3">
          ...
          <span class="text-sm text-text-primary">{{ t("signupPrompt.benefits.recover") }}</span>
        </div>
        <div class="flex items-center gap-3">
          <AppIcon name="check" :size="14" class="text-green-600 dark:text-green-400" />
          <span class="text-sm text-text-primary">{{ t("signupPrompt.benefits.track") }}</span>
        </div>
        <div class="flex items-center gap-3">
          <AppIcon name="check" :size="14" class="text-green-600 dark:text-green-400" />
          <span class="text-sm text-text-primary">{{ t("signupPrompt.benefits.profile") }}</span>
        </div>
        <div class="flex items-center gap-3">
          <AppIcon name="check" :size="14" class="text-green-600 dark:text-green-400" />
          <span class="text-sm text-text-primary">{{ t("signupPrompt.benefits.upcoming") }}</span>
        </div>
      </div>
    </div>
