...
// La semaine en cours, en sept pastilles du dimanche au Chabbat : réussie,
// entamée, manquée, en pause, à venir. Voir ses trous motive plus qu'un
...
defineProps<{ days: DayCell[]; compact?: boolean }>();
  ...
  <ol class="flex items-center justify-between gap-1" :aria-label="t('dailyReading.week.title')">
    ...
    >
      ...
      <span v-if="!compact" class="text-[10px] text-text-secondary/70">
        {{ t(`dailyReading.week.days.${index}`) }}
      </span>
    </li>
  </ol>
</template>
