...
type Meta = {
  ...
  image?: string;
  /**
   * Données structurées (schema.org) de la page, posées dans le <head>. Les
   * pages statiques en ont depuis le prérendu ; les pages dynamiques n'en
   * avaient aucune, un chiour n'était donc jamais reconnu comme un audio.
   */
...
function injectMeta(shell: string, meta: Meta): string {
  ...
  if (meta.jsonLd?.length) {
    const blocks = meta.jsonLd
      // `<` échappé : un « </script> » dans un nom fermerait la balise.
      .map(
        (obj) =>
...
/** Le fil d'Ariane schema.org d'une page dynamique. */
...
type ChiourPreview = {
  ...
  description: string;
  auteur: string | null;
  /** L'URL de l'audio et sa durée (secondes), pour les données structurées. */
  ...
  const data: ChiourPreview[] = snap.docs
    ...
    .map((d) => {
      ...
      return {
        ...
        duration: (x.duration as number | null) ?? null,
      };
    });
  ...
  return {
    ...
    jsonLd: [
      breadcrumb([
        ...
        { name: data.name, path: `/share-reading/session/${slug}` },
      ]),
    ],
  };
}
...
async function resolveChiourMeta(slug: string): Promise<Meta | null> {
  ...
  if (directData && directData.published !== false && directData.name) {
    found = {
      ...
      mediaUrl: (directData.mediaUrl as string | null) ?? null,
      duration: (directData.duration as number | null) ?? null,
    };
  } else {
  ...
  const url = `${SITE_URL}/chiourim/${slug}`;

  // Un chiour est un enregistrement audio : le dire à schema.org (AudioObject)
  // permet aux moteurs de le classer comme tel, avec sa durée et son auteur.
  const audio: Record<string, unknown> = {
    ...
    publisher: { "@type": "Organization", name: "Petite Jérusalem", url: `${SITE_URL}/` },
  };
  if (found.auteur) audio.author = { "@type": "Person", name: found.auteur };
  if (found.mediaUrl) audio.contentUrl = found.mediaUrl;
  if (found.duration) audio.duration = isoDuration(found.duration);
  ...
  return {
    ...
    jsonLd: [
      breadcrumb([
        ...
        { name: found.name, path: `/chiourim/${slug}` },
      ]),
      audio,
    ],
  };
...
/** 754 secondes → « PT12M34S », la durée telle que schema.org l'attend. */
