...
async function findProfileNamed(name) {
  ...
  const match = found.data.find((entry) => entry.attributes?.name === name) ?? null;
  // La liste porte normalement le contenu du profil. Si Apple l'omettait, il
  // faudrait aller le chercher : sans lui, rien à écrire sur le runner, et le
  // rattrapage échouerait là où il est censé sauver le run.
  if (match && !match.attributes?.profileContent) {
    const full = await api("GET", `/v1/profiles/${match.id}`);
    return full.data;
  }
  return match;
}
